...
type OrderItemInput = {
    ...
    pricePerBaseQty: Prisma.Decimal;
...
export async function POST(req: NextRequest) {
    ...
    for (const item of data.items) {
        ...
        if (!allowedUnits.includes(orderUnit)) {
            return NextResponse.json({ error: `Invalid unit for ${product.name}` }, { status: 400 });
        }
        ...
        if (baseQty.gt(product.stockBaseQty)) {
        ...
        orderItems.push({
            ...
            orderUnit,
            baseQty,
            pricePerBaseQty: product.pricePerBaseQty,
            lineTotal,
        });
    }
    ...
    const order = await prisma.$transaction(async (tx) => {
        for (const item of orderItems) {
            ...
            if (!product || item.baseQty.gt(product.stockBaseQty)) {
